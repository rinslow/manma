...
public class RedBlackTree<T extends Comparable<T>> {
     ...
     */
    public List<T> getGreaterOrEqualsThan(T key, Integer maxReturned) {
        List<T> list = new ArrayList<T>();
        getGreaterOrEqualsThan(root, key, list);
        return list.subList(0, Math.min(maxReturned, list.size()));
    }


    private void getGreaterOrEqualsThan(RedBlackNode<T> node, T key,
                                List<T> list) {
        if (isNil(node)) {
            return;
        } else if (node.key.compareTo(key) >= 0) {
            getGreaterOrEqualsThan(node.left, key, list);
            list.add(node.key);
